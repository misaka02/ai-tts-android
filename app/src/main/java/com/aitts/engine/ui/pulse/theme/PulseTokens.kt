 ...
 * 支持 8 种调色板与 AMOLED 纯黑底层及明亮模式动态实时渲染
 */
object PulseTokens {
    // 🎨 静态缺省色
    val DefaultCyanElectric = Color(0xFF00F2FE)
    val DefaultSonicBlue = Color(0xFF4FACFE)
    val DefaultMagentaLaser = Color(0xFFFF0844)
    ...
    val defaultColors = PulseDynamicColors(
        ...
        accent = Color(0xFFFF0844),
        canvasDeep = Color(0xFF06080F),
        surfaceDark = Color(0xFF0D111C),
        surfaceCard = Color(0xFF131826),
        surfaceElevated = Color(0xFF1A2133),
        surfaceCardActive = Color(0xFF1E283D),
        textPrimary = Color(0xFFFFFFFF),
        textSecondary = Color(0xFFA0AEC0),
        textTertiary = Color(0xFF64748B),
        borderSubtle = BorderStroke(1.dp, Color(0xFF222B42)),
        borderActive = BorderStroke(1.5.dp, Color(0xFF00F2FE)),
        brandPillGradient = Brush.horizontalGradient(listOf(Color(0xFF00F2FE), Color(0xFF4FACFE)))
    )
    ...
    fun resolveDynamicColors(palette: String, isAmoledPureBlack: Boolean, themeMode: String = "DARK"): PulseDynamicColors {
        ...
        val (primary, secondary, accent) = when (palette.uppercase()) {
            "EMERALD_JADE" -> if (isLight) Triple(Color(0xFF059669), Color(0xFF10B981), Color(0xFFD97706))
                             else Triple(Color(0xFF00FF87), Color(0xFF60EFA0), Color(0xFFFFB300))
        ...
        val borderSubtle = when {
            ...
            else -> BorderStroke(1.dp, Color(0xFF222B42))
        }
        val borderActive = BorderStroke(1.5.dp, primary)
